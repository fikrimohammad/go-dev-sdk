...
func TestConfig_Validate_Valid(t *testing.T) {
	...
	c := Config{Endpoints: []string{"localhost:4222"}, Topic: "test-topic"}.SetDefaults()
...
func TestConfig_Validate_Errors(t *testing.T) {
	...
	cases := []struct {
		...
		{"empty topic", Config{Endpoints: []string{"localhost:4222"}}, ErrTopicRequired},
		{"topic with dots", Config{Endpoints: []string{"localhost:4222"}, Topic: "oec.order.event"}, ErrInvalidTopic},
	}
